...
export const addCategory = createAsyncThunk("categories/addCategory",async ({formData,resetForm},{rejectWithValue}) =>{
  try{
    const res= await axios.post("http://localhost:3046/category",formData,{headers:{Authorization:localStorage.getItem('token')}});
    resetForm();
    console.log(res.data)
    return res.data;
  ...
  async ({ categoryObj, resetForm }, { rejectWithValue }) => {
    try {
      const res = await axios.put(
        ...
        categoryObj,{headers:{Authorization:localStorage.getItem('token')}}
      );
      resetForm();
      return res.data;
    } catch (err) {
...
export const removeCategory = createAsyncThunk("categories/removeCategory", async (id) =>{
  try{
    console.log(id)
    const res = await axios.delete(`http://localhost:3046/category/${id}`,{headers:{Authorization:localStorage.getItem('token')}});
    console.log(res.data)
    return res.data
  }catch(err){
    console.log(err)
...
const CategorySlice = createSlice({
  name: "categories",
  initialState: { categoryData: [], loading: false, serverError: null,editId:null },
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(updateCategory.fulfilled,(state,action)=>{
      const index = state.categoryData.findIndex(ele=>ele._id ===action.payload._id)
